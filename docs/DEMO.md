...
**Agent routine (what you should do):** `boot` or `boot(pack=overlay)` = **routine working-set load**.  
*Smoke may still probe `pack=attach` for who+map coverage — that is a gate check, not the day-to-day entry imprint.*

### 2. Draft only (no permanent write yet)
