...
import { formatBootResult, runBoot } from "./tools/boot.js";
...
async function main() {
  ...
  server.registerTool(
    ...
    {
      description:
        "Load a small fixed boot view (not a vault dump). pack: pulse (default) | attach (pulse+who+index, 12k budget) | law | map_intent | map_index | who | full. Prefer attach or pulse over full when dialed-in.",
      inputSchema: {
        pack: z
          .enum([
            "pulse",
          ...
          .optional()
          .describe(
            "Boot view. Default pulse. attach = dialed-in recipe. full = sterile four-file pack."
          ),
      },
    },
    async ({ pack }) => {
      try {
        ...
        const human = formatBootResult(result);
        return textResult(
          formatEnvelope(
  ...
  server.registerTool(
    ...
    {
      description:
        "Narrow live glance: personal focus (Active context) plus optional project “what’s true now” for one index intent intent. Ambiguous intents return choices — never merges. No counters. No whole-vault dump.",
      inputSchema: {
        intent: z
          .string()
          .optional()
          .describe(
            "Optional index Intent phrase. Omit for personal focus only. If ambiguous, returns choices."
          ),
      },
    },
    async ({ intent }) => {
  ...
  server.registerTool(
    ...
    {
      description:
        "Classify a permanent write and stage a proposal. Never writes the vault. Returns class + proposal_id. Not-allowed paths → no id. Operator must apply_write(accept:true) for drafts that need accept.",
      inputSchema: {
        path: z
          .string()
  ...
  server.registerTool(
    ...
    {
      description:
        "Apply a staged proposal after operator accept. Requires proposal_id and accept: true. Sole MCP permanent path for drafts that need accept.",
      inputSchema: {
