...
export function runStatus(root: string, intent?: string | null): StatusResult {
  ...
  if (!q) {
    return {
      ...
      hub_now: null,
      hub_path: null,
      note: "No intent — Session Now only. Pass intent to load one Hub Now.",
    };
  }
  ...
  if (resolved.matches.length === 0) {
    return {
      ...
      hub_now: null,
      hub_path: null,
      note: `No Map Intent match for: ${q}`,
    };
  }
  ...
  if (match.paths.length > 1) {
    // Prefer first path only for Hub Now — contract: at most one hub body
