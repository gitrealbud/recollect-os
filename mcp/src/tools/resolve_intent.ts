...
export function runResolveIntent(
  ...
  if (rows.length === 0) {
    return {
      query: q,
      matches: [],
      hint: "No Intent table rows found — open short-index view: boot(pack=map_intent).",
    };
  }
  ...
  if (scored.length === 0) {
    return {
      query: q,
      matches: [],
      hint: "No Intent match — try short-index view boot(pack=map_intent) or index slice boot(pack=map_index).",
    };
  }
