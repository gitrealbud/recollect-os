...
test("findKitRoot locates recollect-os", () => {
  ...
  for (const name of ["Map.md", "Hub.md", "DOC-SYSTEM.md", "ATTACH-GRID.md"]) {
    assert.ok(
      fs.existsSync(path.join(kit, "docs", name)),
      `kit missing docs/${name}`
    );
  }
  const hub = fs.readFileSync(path.join(kit, "docs", "Hub.md"), "utf8");
  assert.match(hub, /^## Now\s*$/m);
});
...
test("init creates vault and smoke passes", () => {
  ...
  assert.ok(fs.existsSync(path.join(dir, "docs", "Map.md")));
  assert.ok(fs.existsSync(path.join(dir, "docs", "Hub.md")));
  assert.ok(fs.existsSync(path.join(dir, "docs", "RELATIONSHIP-SCHEMA.md")));
  const rule = fs.readFileSync(
    path.join(dir, ".cursor", "rules", "recollect.mdc"),
    "utf8"
  );
  assert.match(rule, /docs\/Map\.md/);
  assert.match(rule, /docs\/Hub\.md/);
...
test("status prints Now surfaces and health line", () => {
  ...
  assert.match(r.body, /Session Now/);
  assert.match(r.body, /Hub Now/);
