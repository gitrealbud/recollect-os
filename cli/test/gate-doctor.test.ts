...
`
  ...
  assert.ok(
    report.lines.some((l) => l.startsWith("CANDIDATE") && l.includes("Essay.md")),
    report.lines.join("\n")
  );
...
test("promote --stage proposes onto public root", async () => {
  const { runPromote } = await import("../src/promote.js");
  const { runCliAccept } = await import("../src/accept.js");
  const priv = fs.mkdtempSync(path.join(os.tmpdir(), "recollect-priv-"));
  const pub = fs.mkdtempSync(path.join(os.tmpdir(), "recollect-pub-"));
  initVault({ target: priv });
  initVault({ target: pub });
  const rel = "vault/Essay.md";
  fs.writeFileSync(
    path.join(priv, rel),
    `---
domain: personal
...
`
  ...
  const staged = runPromote({
    root: priv,
    stage: true,
    path: rel,
    publicRoot: pub,
  });
  assert.equal(staged.ok, true, staged.lines.join("\n"));
  assert.ok(staged.lines.some((l) => /proposal|staged|accept/i.test(l)));

  const accepted = runCliAccept(pub, { latest: true });
  assert.equal(accepted.ok, true, accepted.body);
  const out = fs.readFileSync(path.join(pub, rel), "utf8");
  assert.ok(out.includes("Public thought"));
