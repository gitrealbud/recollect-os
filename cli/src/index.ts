...
Usage:
  ...
  recollect-os promote --dry-run [--root dir] [--public-root dir]
                                    Membrane report + visibility:public candidates (no write)
  recollect-os promote --stage --path vault/…/note.md --public-root dir [--root dir]
                                    Propose one scrubbed note onto the public thoughts vault
  recollect-os help

Known limit: tools cannot block free-form host file edits. Permanent path = draft then accept.
Dual repo: private vault + optional public thoughts vault — docs/DUAL-REPO.md (never auto-sync).
Other hosts: see README / docs/ATTACH.md.
...
Env:
  ...
  RECOLLECT_PUBLIC_ROOT  Public thoughts vault for promote --stage / report
...
async function main() {
  ...
  if (cmd === "promote") {
    const root =
      ...
      defaultInitTarget();
    const publicRoot =
      flagValue(argv, "--public-root") ?? process.env.RECOLLECT_PUBLIC_ROOT;
    const stage = flags.has("--stage");
    const relPath = flagValue(argv, "--path");
    if (!stage && !flags.has("--dry-run")) {
      console.error(
        "promote requires --dry-run (report) or --stage --path … --public-root … (no auto-sync)"
      );
      process.exit(1);
    }
