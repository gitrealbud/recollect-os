...
const LAW_DOCS = [
  ...
  "START-GUIDE.md",
  "WORKING-GUIDE.md",
  "INTELLIGENCE-CARD.md",
  "RESEARCH-AND-ANALYSIS.md",
...
- Permanent changes: propose → human accepts. Do not silent-write paths the tools refuse.
- After onboarding ends: short index → that note’s “what’s true now” section only (at most two notes).
- Onboarding starts by default until the human ends it — see docs/START-GUIDE.md and docs/INTELLIGENCE-CARD.md.
...
Permanent changes: propose → you accept. Some paths tools will not apply.
...
*(Controlled relationship labels only — see RELATIONSHIP-SCHEMA. Do not put live focus on the index.)*
